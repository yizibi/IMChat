    ...
    if ([self setupGLOnce]) {
        ...
        if (![self tryLockGLActive]) {
            if (0 == (_tryLockErrorCount % 100)) {
                NSLog(@"NTESGLView:display: unable to tryLock GL active: %d\n", _tryLockErrorCount);
            }
            _tryLockErrorCount++;
            return;
        }
        ...
        if (!_didStopGL) {
            if (_context == nil) {
                NSLog(@"NTESGLView: nil EAGLContext\n");
                return;
            }
            EAGLContext *prevContext = [EAGLContext currentContext];
            [EAGLContext setCurrentContext:_context];
...
- (void)displayInternal: (NTESGLVoutOverlay *)overlay
{
    ...
    if (![self setupDisplay:overlay]) {
        NSLog(@"NTESGLView: setupDisplay failed\n");
        return;
    }
...
{
    NSLog(@"NTESGLView:applicationWillEnterForeground: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:NO];
}

- (void)applicationDidBecomeActive
{
    NSLog(@"NTESGLView:applicationDidBecomeActive: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:NO];
}
...
{
    NSLog(@"NTESGLView:applicationWillResignActive: %d", (int)[UIApplication sharedApplication].applicationState);
...
{
    NSLog(@"NTESGLView:applicationDidEnterBackground: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:YES];
}

- (void)applicationWillTerminate
{
    NSLog(@"NTESGLView:applicationWillTerminate: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:YES];
}
