...
- (void)startByCaller{
    ...
    NSLog(@"%@",[NIMSDK sharedSDK].currentLogFilepath);
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (wself.userHangup) {
            NSLog(@"Netcall request was cancelled before, ignore it.");
            return;
        }
        ...
        [[NIMSDK sharedSDK].netCallManager start:callees type:wself.callInfo.callType option:option completion:^(NSError *error, UInt64 callID) {
            ...
            if (!error && wself) {
                ...
                if (error) {
                     ...
                     [wself.navigationController.view nimkit_makeToast:@"连接失败"
                                                      duration:2
        ...
        if (!error) {
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    if (wself.chatRoom.count == 1) {
                        [wself.navigationController.view nimkit_makeToast:@"通话失败"
                                                          duration:2
                                                          position:NIMKitToastPositionCenter];
                        [wself hangup];
                    }
...
- (void)onControl:(UInt64)callID
    ...
    switch (control) {
            ...
            if (room && !room.count) {
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    ...
                    if (wself && room && room.count == 1) {
                        ...
                        [wself.navigationController.view nimkit_makeToast:@"无人接听"
                                                          duration:2
                                                          position:NIMKitToastPositionCenter];
                        [wself dismiss:nil];
                    }
        ...
        case NIMNetCallControlTypeStartLocalRecord:
            [self.view nimkit_makeToast:@"对方开始了本地录制"
                        duration:1
                        position:NIMKitToastPositionCenter];
            break;
        case NIMNetCallControlTypeStopLocalRecord:
            [self.view nimkit_makeToast:@"对方结束了本地录制"
                        duration:1
                        position:NIMKitToastPositionCenter];
            break;
        default:
...
- (void)onResponse:(UInt64)callID from:(NSString *)callee accepted:(BOOL)accepted{
    if (self.callInfo.callID == callID) {
        if (!accepted) {
            self.chatRoom = nil;
            [self.navigationController.view nimkit_makeToast:@"对方拒绝接听"
                                             duration:2
                                             position:NIMKitToastPositionCenter];
...
- (void)onResponsedByOther:(UInt64)callID
    ...
    [self.view nimkit_makeToast:@"已在其他端处理"
                duration:2
                position:NIMKitToastPositionCenter];
    [self dismiss:nil];
}
                    ...
                    callID:(UInt64)callID
{
    ...
    if (self.callInfo.callID == callID) {
        ...
        [self.view nimkit_makeToast:[NSString stringWithFormat:@"录制发生错误: %zd", error.code]
                    duration:2
                    position:NIMKitToastPositionCenter];
...
- (void)onNTESTimerFired:(NTESTimerHolder *)holder{
    if (holder == self.diskCheckTimer) {
        ...
        if (!_calleeResponsed) {
            [self.navigationController.view nimkit_makeToast:@"接听超时"
                                              duration:2
                                              position:NIMKitToastPositionCenter];
            [self response:NO];
        }
...
- (void)saveToPhotosAlbum:(NSURL *)fileURL
{
    if (fileURL) {
        ...
        [library writeVideoAtPathToSavedPhotosAlbum:fileURL
            ...
            [wself.navigationController.view nimkit_makeToast:toast
                                              duration:3
                                              position:NIMKitToastPositionCenter];
        }];
