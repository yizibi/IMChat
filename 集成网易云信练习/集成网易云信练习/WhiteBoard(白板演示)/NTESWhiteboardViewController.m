...
//    NSLog(@"receive app data:%@", cmdString);
          ...
          forSession:(NSString *)sessionID
{
    if (sessionID == _sessionID) {
        ...
        [self.view.window nimkit_makeToast:[NSString stringWithFormat:@"%@: %@", user, controlInfo]
                           duration:2
                           position:NIMKitToastPositionBottom];
    }
}
...
- (void)responseRTS:(BOOL)accepted
{
    ...
    [[NIMSDK sharedSDK].rtsManager responseRTS:_sessionID
        ...
        if (error) {
            ...
            NSLog(@"RTSDemo: send response failed %zd!!!", error.code);
            [wself dismissAfter:2];
        }
    }];
}
...
//        NSLog(@"++++++send cmd id %llu", _refPacketID);
...
- (void)sendRTSData:(NSString *)data
{
    ...
    if (!success) {
        [self.view.window nimkit_makeToast:@"数据发送失败" duration:1 position:NIMKitToastPositionBottom];
    }
}
...
{
    [self.view.window nimkit_makeToast:toast duration:3 position:NIMKitToastPositionCenter];
}
